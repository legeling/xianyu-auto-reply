...
# 确保 .env.remote 中某个 key 存在且非空；缺失或为空时写入随机值（已存在的值保持不变）
ensure_env_secret() {
    local key="$1"
    local current
    ...
    if [ -z "$current" ]; then
        ...
        echo -e "${YELLOW}[安全] 已为 ${key} 生成随机密钥，请妥善保管 $ENV_FILE${NC}"
    fi
}
...
REDIS_PORT=6379
# 必填：远程 Redis 的实际密码（无默认值，请勿使用弱口令）
REDIS_PASSWORD=
REDIS_DB=0
...
services:
  ...
  backend-web:
    ...
    environment:
      ...
      - MYSQL_PASSWORD=${MYSQL_PASSWORD:?MYSQL_PASSWORD must be set in .env.remote}
      - MYSQL_DATABASE=${MYSQL_DATABASE:-xianyu_data}
      - REDIS_HOST=${REDIS_HOST}
      - REDIS_PORT=${REDIS_PORT:-6379}
      - REDIS_PASSWORD=${REDIS_PASSWORD:?REDIS_PASSWORD must be set in .env.remote}
      - REDIS_DB=${REDIS_DB:-0}
      - BACKEND_WEB_PORT=8089
      - HOST=0.0.0.0
      - JWT_ALGORITHM=HS256
      ...
      - INTERNAL_API_TOKEN=${INTERNAL_API_TOKEN:?INTERNAL_API_TOKEN must be set in .env.remote}
      - STATIC_DIR=/app/static
      - BACKUP_DIR=/app/backups
      - BACKEND_WEB_PUBLIC_URL=${BACKEND_WEB_PUBLIC_URL:-}
      # 上游 https 证书已于 2026-04-24 过期，暂保留 http；证书续期后请改为 https://backend.zhinianboke.com
      - CARD_DOCK_BASE_URL=${CARD_DOCK_BASE_URL:-http://backend.zhinianboke.com}
      # 必填：向上游申请的鉴权 key，无默认值，请在 .env.remote 中配置
...
mysql_password=$(grep -E "^MYSQL_PASSWORD=" "$ENV_FILE" 2>/dev/null | cut -d '=' -f2- | tr -d '\r')
redis_password=$(grep -E "^REDIS_PASSWORD=" "$ENV_FILE" 2>/dev/null | cut -d '=' -f2- | tr -d '\r')
...
if [ -z "$mysql_password" ]; then
    echo -e "${RED}错误: 请先在 $ENV_FILE 中填写 MYSQL_PASSWORD（远程数据库密码，无默认值）${NC}"
    exit 1
fi
if [ -z "$redis_password" ]; then
    echo -e "${RED}错误: 请先在 $ENV_FILE 中填写 REDIS_PASSWORD（远程 Redis 密码，无默认值）${NC}"
    exit 1
fi
